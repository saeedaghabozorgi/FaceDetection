...
def train_test_spit():
    ...
    ids_train_set, ids_test_set = training_idx.reshape(-1,1), test_idx.reshape(-1,1)
    print(features_train_set.shape)
    return(features_train_set, features_test_set,labels_train_set, labels_test_set,ids_train_set, ids_test_set)
def train():
    features_train_set, features_test_set,labels_train_set, labels_test_set,ids_train_set, ids_test_set = train_test_spit()
    graph1 = tf.Graph()
    with graph1.as_default():
        ...
        with tf.device('/cpu:0'):
            ...
            features_data_ph = tf.placeholder(tf.float32, shape = [None, face.IMAGE_SIZE,face.IMAGE_SIZE,1] , name ='features_data_ph')
            labels_data_ph = tf.placeholder(tf.int32, shape = [None,1], name ='labels_data_ph')
            ids_data_ph = tf.placeholder(tf.int32, shape = [None,1], name ='ids_data_ph')
            
            # Dataset
            train_dataset = tf.data.Dataset.from_tensor_slices((ids_data_ph,features_data_ph, labels_data_ph)).batch(batch_size_ph)
            test_dataset = tf.data.Dataset.from_tensor_slices((ids_data_ph, features_data_ph, labels_data_ph)).batch(batch_size_ph)
            ...
            print('-------------')
            print('image shape:')
            id_tensor, image_tensor, labels_tensor = iterator.get_next()
            ...
            train_op, lr = face.train(loss, global_step)
                ...
                for epoch in range(11):
                    ...
                    while True:
                        try:
                            ...
                            _, learningRate, loss_value, sample_img , sample_label, pred_value = sess.run([train_op, lr, loss,image_tensor, labels_tensor, tf.nn.softmax(logits)])
                            train_pred_values.append(pred_value)
                            accuracy = np.mean(sample_label==np.argmax(pred_value,1))
                            print('Epoch {}, batch {}, step {}, lr {:.4f}, loss {:.2f}, acc {:.2f} | Sample: {}, Pred value: {}'.format(epoch,  batch, step, learningRate, loss_value, accuracy, sample_label[0], pred_value[0]))
                            batch += 1
